...
def upgrade_plugin_data():
    if plugin_data_path_v1.exists() and plugin_data_path_v1.is_file():
        ...
        try:
            ...
            logger.error(f"创建转换后的插件配置文件失败，请检查是否有权限读取和写入 {plugin_config_path}")
            write_success = False
        ...
        try:
            ...
            logger.error(f"创建转换后的插件数据文件失败，请检查是否有权限读取和写入 {plugin_config_path}")
            write_success = False
