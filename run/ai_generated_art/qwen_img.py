...
censor_words = ["大便", "屎", "shit", "排泄物", "排泄", "粪便", "poop", "一坨"]

async def generate_and_save_image(prompt: str, cookies: list) -> str:
    ...
    for cookie_idx, cookie in enumerate(cookies, 1):
        print(f"尝试使用第 {cookie_idx}/{len(cookies)} 个Cookie")
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            try:
                ...
                if not (submit_result.get("Code") == 200 and submit_result.get("Success")):
                    ...
                    print(f"第 {cookie_idx} 个Cookie提交失败: {error_msg}")
                    continue  # 尝试下一个cookie
                    
                task_id = submit_result["Data"]["data"]["taskId"]
                print(f"第 {cookie_idx} 个Cookie任务提交成功，taskId: {task_id}")

            except httpx.RequestError as e:
                print(f"第 {cookie_idx} 个Cookie提交请求失败: {str(e)}")
                continue  # 尝试下一个cookie
            except (KeyError, ValueError) as e:
                print(f"第 {cookie_idx} 个Cookie解析提交结果失败: {str(e)}")
                continue  # 尝试下一个cookie
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if task_status == "SUCCEED":
                        ...
                        print(f"第 {cookie_idx} 个Cookie任务失败({task_status}): {error_msg}")
                        break
            ...
            print(f"第 {cookie_idx} 个Cookie轮询超时，尝试下一个Cookie")

    return f"所有Cookie尝试均失败，请检查Cookie有效性"
...
# 这里跑一次图f12自己拿cookie
    ...
    results = await generate_and_save_image(prompt=prompt, cookies=cookies)
