    ...
    if len(image_list) != 1:
        json_check['pic_path'] = await manshuo_draw([
            {'type': 'avatar', 'subtype': 'common', 'img': [note_data['user']['avatar']], 'upshift_extra': 25,
             'content': [{'name': note_data['user']['nickname'], 'time': video_time}], 'type_software': 'xhs','label':label_list }, image_list, [context]])
    else:
        json_check['pic_path'] = await manshuo_draw([
            {'type': 'avatar', 'subtype': 'common', 'img': [note_data['user']['avatar']], 'upshift_extra': 25,
             'content': [{'name': note_data['user']['nickname'], 'time': video_time}], 'type_software': 'xhs', },
